const SpatialIcons = {
    ...
    render(name, options = {}) {
        const { size = 24, strokeWidth = 1.5, className = '', animate = false } = options;
        const iconPath = this.icons[name];
        if (!iconPath) {
            console.warn(`SpatialIcons: Icon "${name}" not found`);
            return '';
        }
        const animationClass = animate ? 'spatial-icon--animated' : '';
        return `<svg 
      ...
      class="spatial-icon spatial-icon--${name} ${animationClass} ${className}"
      aria-hidden="true"
    >${iconPath}</svg>`;
    },
    ...
    init() {
        document.querySelectorAll('[data-icon]').forEach(el => {
            ...
            const animate = el.dataset.iconAnimate === 'true';
            el.innerHTML = this.render(iconName, { size, animate });
        });
        console.log('✨ SpatialIcons initialized');
    },
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => SpatialIcons.init());
} else {
    SpatialIcons.init();
}

if (typeof module !== 'undefined' && module.exports) {
    module.exports = SpatialIcons;
}
